    ...
    def execute(self, params: dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            if pop_dialog_content:
                return OperationResult(
                    success=False,
                    message=f"市价买入操作失败，错误：{pop_dialog_content}",
                )

            msg = f"市价买入操作成功，买入策略：{selected_str}"
            if execution_strategy not in EXECUTION_STRATEGIES:
                msg = f"市价买入操作成功，策略编号 {execution_strategy} 无效，已使用默认策略：{selected_str}"
            elif selected_strategy != execution_strategy:
                msg = f"市价买入操作成功，买入策略在标的{stock_code}下不支持，已使用默认策略：{selected_str}"

    ...
    def execute(self, params: Dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            if pop_dialog_content:
                return OperationResult(success=False, message=f"市价卖出操作失败，错误：{pop_dialog_content}")

            msg = f"市价卖出操作成功，卖出策略{waiting_for_select_str}"
            if not is_valid_execution_strategy:
                msg = f"市价卖出操作成功，卖出策略在标的{stock_code}情况下不支持，已经使用默认策略：{waiting_for_select_str}提交）"

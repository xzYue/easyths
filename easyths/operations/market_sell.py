...
class MarketSellOperation(BaseOperation):
    """市价卖出股票操作"""

    def _get_metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="MarketSellOperation",
            version="1.0.0",
            description="市价卖出股票操作",
            author="noimank",
    ...
    def execute(self, params: dict[str, Any]) -> OperationResult:
        ...
        try:
            ...
            # 选择成交策略：按名称内容匹配（客户端策略数量/顺序不定，不能按编号定位）
            texts = [i[0] for i in list_box.texts()]
            texts2str = "".join(texts)
            if "不支持市价委托" in texts2str:
                return OperationResult(
                    success=False, message=f"标的：{stock_code}，不支持市价委托"
                )

            resolved = resolve_strategy(texts, execution_strategy)
            if resolved is None:
                return OperationResult(
                    success=False,
                    message=f"标的：{stock_code}，不含可用市价成交策略"
                    f"（请求策略 {EXECUTION_STRATEGIES.get(execution_strategy, '未知')} "
            ...
            if pop_dialog_content:
                return OperationResult(
                    success=False,
                    message=f"市价卖出操作失败，错误：{pop_dialog_content}",
                )

            msg = f"市价卖出操作成功，卖出策略：{selected_str}"
            if execution_strategy not in EXECUTION_STRATEGIES:
                msg = f"市价卖出操作成功，策略编号 {execution_strategy} 无效，已使用默认策略：{selected_str}"
            elif selected_strategy != execution_strategy:
                msg = f"市价卖出操作成功，卖出策略在标的{stock_code}下不支持，已使用默认策略：{selected_str}"

            return OperationResult(
                success=True,
                message=msg,
                data={"stock_code": stock_code, "quantity": quantity, "msg": msg},
            )

        except Exception as e:
            self.logger.exception("市价卖出操作异常", error=str(e))
            return OperationResult(
                success=False,
                message=f"市价卖出操作异常，错误：{e}",
            )
